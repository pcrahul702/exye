...
const SignUpScreen = () => {
    ...
    const handleContinue = async () => {
        if (validateName(name) && validateEmail(email) && validateMobileNo(mobileNo) && validateUsername(username) && validatePassword(password)) {

            let role = 'admin';

            console.log({ username, password, name, email, mobileNo, role });


            const response = await fetch('http://exye-env.eba-mpggjgwv.ap-south-1.elasticbeanstalk.com/register/', {
                method: 'POST',
                headers: {
    ...
    return (
        <KeyboardAvoidingView
            ...
            <View style={styles.inputContainer}>
                ...
                <Text style={styles.tncText}>By continuing, I agree to EXYE's <Text style={[styles.tncText, { fontWeight: '900' }]}>T&Cs</Text>.</Text>
